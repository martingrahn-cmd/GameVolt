// ============================================================
// sb-net.js — room-code online multiplayer transport for Spinburn.
//
...
//
// Spinburn's ball is deterministic between hits, so only a handful of
// events cross the wire per rally: serve/hit launches (full ball state +
// tick), paddle position updates for rendering the rival, and round/score
// sync. This file is transport only — host/join handshake plus typed sends.
// eventsPerSecond is raised for the ~15 paddle updates/sec per client.
//
...
(function (global) {
  ...
  function subscribe(code) {
    var ch = getClient().channel('sb-' + code, { config: { broadcast: { self: false } } });
  ...
  global.SBNet = SBNet;
  if (typeof module !== 'undefined' && module.exports) module.exports = SBNet;
