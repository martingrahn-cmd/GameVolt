...
`;function Oe(e,t=``){let n=Math.max(...e.map(e=>e.image.width)),r=document.createElement(`canvas`);r.width=r.height=n;let i=r.getContext(`2d`,{willReadFrequently:!0}),a=new Uint8Array(n*n*4*e.length);e.forEach((e,t)=>{i.setTransform(1,0,0,1,0,0),i.clearRect(0,0,n,n),e.flipY&&i.setTransform(1,0,0,-1,0,n),i.drawImage(e.image,0,0,n,n),a.set(i.getImageData(0,0,n,n).data,t*n*n*4)});let o=new te(a,n,n,e.length);return o.colorSpace=t,o.wrapS=o.wrapT=F,o.magFilter=_,o.minFilter=v,o.generateMipmaps=!0,o.anisotropy=Math.min(...e.map(e=>e.anisotropy)),o.needsUpdate=!0,o}function ke(e,t,n={}){let r=Oe([t.grassFine.normalMap,t.dirt.normalMap,t.rock.normalMap,t.sand.normalMap]),i=Oe([t.grass.map,t.grassFine.map,t.dirt.map,t.rock.map,t.sand.map,t.scree.map],O),a={uHeightTex:{value:e.heightTex},uNormalTex:{value:e.normalTex},uMacro:{value:t.macro},uLandTex:{value:t.land},uDetailNormals:{value:r},uAlbedoLayers:{value:i},uWorldMin:{value:-e.half},uWorldSize:{value:e.size},uCell:{value:e.cell},uRes:{value:e.res},uSeaLevel:{value:e.seaLevel},uNormalFlip:{value:n.normalFlip??1},uLodDrop:{value:0},uSkirt:{value:1}},o=new m({color:16777215,roughness:1,metalness:0,side:0});return o.name=`terrain-splat`,o.addEventListener(`dispose`,()=>{r.dispose(),i.dispose()}),o.onBeforeCompile=e=>{Object.assign(e.uniforms,a),e.vertexShader=e.vertexShader.replace(`#include <common>`,`#include <common>
`+J).replace(`#include <beginnormal_vertex>`,we).replace(`#include <begin_vertex>`,Y),e.fragmentShader=e.fragmentShader.replace(`#include <common>`,`#include <common>
`+Ee).replace(`#include <map_fragment>`,De).replace(`#include <roughnessmap_fragment>`,`float roughnessFactor = splatRough;`).replace(`#include <normal_fragment_begin>`,`float faceDirection = gl_FrontFacing ? 1.0 : - 1.0;
...
`+Te).replace(`#include <map_fragment>`,`
vec2 wp = vWPos.xz;
...
vec3 normal = normalize((viewMatrix * vec4(splatN, 0.0)).xyz);
vec3 nonPerturbedNormal = normal;`).replace(`#include <normal_fragment_maps>`,``)},i.customProgramCacheKey=()=>`terrain-lite-v3`,i.userData.uniforms=r,i}function je(e){let t={uHeightTex:{value:e.heightTex},uNormalTex:{value:e.normalTex},uWorldMin:{value:-e.half},uCell:{value:e.cell},uLodDrop:{value:.3},uSkirt:{value:0}},n=new y({depthPacking:C});return n.onBeforeCompile=e=>{Object.assign(e.uniforms,t),e.vertexShader=e.vertexShader.replace(`#include <common>`,`#include <common>
`+J).replace(`#include <begin_vertex>`,Y)},n.customProgramCacheKey=()=>`terrain-depth-v3`,n}function Me(e,t=256){let n=new Uint8Array(t*t*4),r=[3,5,4,7];for(let i=0;i<4;i++){let a=e[i],o=r[i];for(let e=0;e<t;e++)for(let r=0;r<t;r++){let s=r/t,c=e/t,l=.5+.5*a.fbm(s*o,c*o,5),u=.5+.5*a.fbm((s+1)*o,(c+1)*o,5),d=Math.min(1-Math.abs(s*2-1),1-Math.abs(c*2-1)),f=d*d*(3-2*d),p=l*f+u*(1-f);p=Math.min(1,Math.max(0,(p-.5)*1.6+.5)),n[(e*t+r)*4+i]=Math.round(p*255)}}let i=new g(n,t,t,M,j);return i.wrapS=i.wrapT=F,i.magFilter=_,i.minFilter=v,i.generateMipmaps=!0,i.needsUpdate=!0,i}function Ne(e){let t=new g(e.data,e.size,e.size,M,j);return t.wrapS=t.wrapT=V,t.magFilter=_,t.minFilter=v,t.generateMipmaps=!0,t.needsUpdate=!0,t}var Pe=`
...
${Te}`).replace(`#include <beginnormal_vertex>`,`
vec3 objectNormal = vec3(0.0, 1.0, 0.0);
#ifdef USE_TANGENT
