...
// ============================================================
// Combined High Scores screen — top 5 per mode, read straight
// from the per-mode localStorage the HighscoreManager writes.
// ============================================================
export class HighScoresScreen {
    ...
    show(onClose) {
        this.onClose = onClose || null;
        const modes = [
            { key: "neo", label: "Neo Synthwave", col: "#ff1dac" },
            { key: "16bit", label: "Fruit Chain", col: "#ffd700" },
            { key: "nokia", label: "Nokia 3310", col: "#9acd32" },
        ];
        let sections = "";
        for (const m of modes) {
            const scores = this._read("snakeHighscores_" + m.key).slice(0, 5);
            let rows = "";
            if (!scores.length) {
                rows = `<div class="sn-hs-empty">No scores yet</div>`;
            } else {
                scores.forEach((s, i) => {
                    const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : "&nbsp;&nbsp;";
                    rows += `<div class="sn-hs-row"><span class="sn-hs-rank">${medal} ${i + 1}</span>` +
                        `<span class="sn-hs-score">${Number(s.score || 0).toLocaleString()}</span></div>`;
                });
            }
            sections += `<div class="sn-hs-sec"><div class="sn-hs-h" style="color:${m.col}">${m.label}</div>${rows}</div>`;
        }
        ...
        div.innerHTML = `
            <style>
                ...
                #snakeHighScores .sn-hs-body{overflow-y:auto;padding:8px 18px 18px}
                #snakeHighScores .sn-hs-sec{margin-top:14px}
                #snakeHighScores .sn-hs-h{font-size:10px;font-weight:700;letter-spacing:2px;margin-bottom:8px}
                #snakeHighScores .sn-hs-row{display:flex;justify-content:space-between;align-items:center;
                    padding:7px 10px;border-radius:6px;background:rgba(0,20,40,0.5);margin-bottom:5px}
                #snakeHighScores .sn-hs-rank{color:#a0a4c0;font-size:9px}
                #snakeHighScores .sn-hs-score{color:#0ff;font-size:9px}
                #snakeHighScores .sn-hs-empty{color:#626580;font-size:8px;padding:6px 10px}
            </style>
            <div class="sn-hs-panel">
                ...
                <div class="sn-hs-body">${sections}</div>
            </div>`;
        document.body.appendChild(div);
        this.element = div;
