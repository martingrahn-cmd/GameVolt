...
export class Input {
  constructor() {
    ...
    this.lastDevice = 'kb';   // 'kb' | 'pad' | 'touch'
    ...
    this._touch = { steer: 0, throttle: 0, brake: 0, airbrake: false };
    ...
    window.addEventListener('blur', () => { this.keys.clear(); this.releaseTouch(); });
  ...
  touchHold(action, value) {
    if (!(action in this._touch)) return;
    this._touch[action] = action === 'steer' ? clampUnit(Number(value) || 0) : !!value;
    if (value) this.lastDevice = 'touch';
  }

  touchPress(action) {
    const code = PAD_CODE[action];
    if (code) this.pressed.add(code);
    else if (action === 'confirm') this.pressed.add('Enter');
    else if (action === 'back') this.pressed.add('Backspace');
    this.lastDevice = 'touch';
  }

  releaseTouch() {
    this._touch.steer = this._touch.throttle = this._touch.brake = 0;
    this._touch.airbrake = false;
    const stick = typeof document !== 'undefined' && document.getElementById('touch-stick');
  ...
  update(dt) {
    ...
    const t = this._touch;
    const steerTarget = clampUnit((right ? 1 : 0) - (left ? 1 : 0) + g.steer + t.steer);
    this.steer = ramp(this.steer, steerTarget, dt, T.STEER_RISE, T.STEER_RELEASE);
    this.throttle = ramp(this.throttle, Math.max(up ? 1 : 0, g.throttle, t.throttle), dt, T.THROTTLE_RISE, T.THROTTLE_RELEASE);
    this.brake = ramp(this.brake, Math.max(down ? 1 : 0, g.brake, t.brake), dt, T.THROTTLE_RISE * 1.5, T.THROTTLE_RELEASE * 1.5);
    this.airbrake = kbAir || g.airbrake || t.airbrake;
  }
  ...
  _bindTouchControls() {
    ...
    if (zone && stick) {
      ...
      const move = (e) => {
        ...
        const dx = Math.max(-radius, Math.min(radius, e.clientX - (rect.left + rect.width / 2)));
        const dy = Math.max(-radius, Math.min(radius, e.clientY - (rect.top + rect.height / 2)));
        this.touchHold('steer', dx / radius);
        stick.style.transform = `translate(${dx}px, ${dy}px)`;
        e.preventDefault();
      };
      const end = (e) => {
        if (pointer !== e.pointerId) return;
        pointer = null; this.touchHold('steer', 0); stick.style.transform = '';
      };
      zone.addEventListener('pointerdown', (e) => {
        pointer = e.pointerId;
        try { zone.setPointerCapture?.(pointer); } catch { /* synthetic/older touch event */ }
        move(e);
      });
    ...
    document.querySelectorAll('[data-touch-hold]').forEach((button) => {
      let pointer = null;
      const action = button.dataset.touchHold;
      const end = (e) => {
        if (pointer !== e.pointerId) return;
        pointer = null; button.classList.remove('is-held'); this.touchHold(action, 0);
      };
      button.addEventListener('pointerdown', (e) => {
        pointer = e.pointerId;
        try { button.setPointerCapture?.(pointer); } catch { /* synthetic/older touch event */ }
        button.classList.add('is-held'); this.touchHold(action, 1); e.preventDefault();
      });
    ...
    document.querySelectorAll('[data-touch-action]').forEach((button) => {
      button.addEventListener('pointerdown', (e) => {
        button.classList.add('is-held'); this.touchPress(button.dataset.touchAction); e.preventDefault();
      });
      const end = () => button.classList.remove('is-held');
      button.addEventListener('pointerup', end);
      button.addEventListener('pointercancel', end);
      button.addEventListener('lostpointercapture', end);
    });
