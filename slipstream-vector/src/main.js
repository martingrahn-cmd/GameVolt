...
// Give an existing GameVolt session a brief chance to restore cloud progress
// before the menu reads the sv-* keys. Offline play never waits more than 1.5s.
await Promise.race([portal.init(), new Promise((resolve) => setTimeout(resolve, 1500))]);
...
const GLYPHS = {
  confirm:  { kb: 'ENTER', xbox: 'A', ps: '&#10005;', touch: 'TAP' },          // ✕
  back:     { kb: '&#9003;', xbox: 'B', ps: '&#9675;', touch: 'BACK' },         // ⌫ · ○
  pause:    { kb: 'P', xbox: '&#9776;', ps: 'OPTIONS', touch: '&#10074;&#10074;' },
  respawn:  { kb: 'R', xbox: 'X', ps: '&#9633;', touch: 'RESET' },
  navigate: { kb: '&uarr;&darr;', xbox: 'D-PAD', ps: 'D-PAD', touch: 'TAP' },
  change:   { kb: '&larr;&rarr;', xbox: 'D-PAD', ps: 'D-PAD', touch: 'TAP' },
};
function glyph(action) {
  ...
  const dev = pad ? input.padKind : (touch ? 'touch' : 'kb');
  const label = set[dev] || set.kb;
  const wide = !label.startsWith('&') && label.length > 1; // multi-char word (D-PAD) → pill, not a round button
  return `<span class="kbtn ${pad ? `pad ${input.padKind}` : (touch ? 'touch' : 'kb')}${wide ? ' wide' : ''}">${label}</span>`;
}
...
function wireClicks() {
  ...
  // The title, pilot intro and post-race boards also accept a direct tap.
  document.getElementById('overlay').addEventListener('click', () => {
    if (state === 'intro') input.touchPress('confirm');
  });
  document.getElementById('pilot-intro').addEventListener('click', () => {
    if (introActive) input.touchPress('confirm');
  });
  document.getElementById('results').addEventListener('click', () => {
    if (state === 'finished') input.touchPress('confirm');
  });
}
